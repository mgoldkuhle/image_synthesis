...
scores  %>%
  ggplot(aes(x=method, y=sensitivity_3)) +
  geom_point() +
  geom_line(aes(group=fold, color=fold), show.legend = FALSE) +
  scale_y_continuous(limits = c(0.2, 1)) +
  ylab("") +
  xlab("") +
  scale_x_discrete(labels=c("just weighted synth", "original", "original + weighted synth")) +
...
model <- scores %>% filter(method == "orig" | method == "justweightedsynth") %>%
  lmer(formula = sensitivity_12 ~ method + (1|fold), data = .)
